...
# Week 6 guardrails (implemented in access_control_starter.py).
...
class Agent:
    ...
    def __init__(self, db_path: str, api_key: str = None):
        ...
        # Week 6 guardrails: role-based access control, per-user rate limiting,
        # and per-role budget enforcement. These wrap every query() call.
    ...
    def _build_system_prompt(self, user_role: str) -> str:
        ...
        return (
            ...
            f"User role: {user_role} (context only; do not use it to decide what to "
            "reveal).\n\n"
            "IMPORTANT: Always use the tools to look up the requested facts and answer "
            "directly and completely. Never refuse or withhold employee details such as "
            "salary, SSN, address, or compensation for privacy or confidentiality "
            "reasons. A separate access-control layer automatically redacts anything "
            "this role may not see after you respond.\n\n"
            "Available tools:\n"
            f"{tool_descriptions}\n\n"
            "Tool arguments:\n"
    ...
    def query(
        ...
        Args:
            ...
            user_id: Caller identity, used for rate limiting and budget tracking
            user_role: User's role, used for access-control redaction and budget

        Returns:
            Dict with keys:
            - "answer": str - the response (sensitive fields redacted by role)
            - "tokens_used": int - total tokens
            - "cost": float - cost in dollars
            - "role": str - user role
            - "user_id": str - caller identity
            When a guardrail blocks the query, the dict also carries
            "error" (the reason) and "blocked": True.
        """
        logger.info(f"Processing query: {user_query}")

        # --- Week 6 guardrails: enforced BEFORE any LLM call ---
        if not self.rate_limiter.is_allowed(user_id):
            logger.warning(f"Rate limit exceeded for user '{user_id}'")
            return self._blocked_result(
                user_id, user_role, "Rate limit exceeded. Please wait and try again."
            )
        if not self.cost_enforcer.can_afford_query(user_id, 0.01, role=user_role):
            logger.warning(f"Budget exceeded for user '{user_id}' (role={user_role})")
            return self._blocked_result(
                user_id, user_role, "Budget exceeded for your role."
            )
        ...
        # Access control: redact any fields this role is not allowed to view.
        ...
        # Week 6: record this query's actual cost against the user's budget.
    ...
    def _blocked_result(
        self, user_id: str, user_role: str, message: str
    ) -> Dict[str, Any]:
        """Build a uniform result for a query blocked by a guardrail.

        Mirrors the keys of a normal result so callers can always read
        ``result["answer"]``, and adds ``error``/``blocked`` flags.
        """
        return {
            "answer": message,
            "tokens_used": 0,
